...
#Latest change: update population data to be grouped by age AND sex
...
generate_pal_vectorized <- function(sex_vector, bmi_vector) {
  ...
  mapply(function(sex, bmi_cat) {
    ...
    if(nrow(cat_params) == 0) {
      cat_params <- params %>% slice_tail(n = 1)
    }
...
full_results <- full_results %>%
  mutate(
    qualifies_for_treatment = case_when(
      diabetes_type == 1 ~ FALSE,  # Exclude Type 1 diabetes first
      bmi >= 30 ~ TRUE,  # BMI ≥30 eligibility (except Type 1)
      bmi >= 27 & bmi < 30 & diabetes == 1 & diabetes_type == 2 ~ TRUE,  # BMI 27-30 + Type 2 diabetes
      TRUE ~ FALSE
    ),
    
    # First generate adherence (95% chance based on discontinuation due to adverse effects)
    adheres_to_treatment = case_when(
      qualifies_for_treatment ~ rbinom(n(), 1, 0.95) == 1,
      TRUE ~ FALSE
    ),
    
    # Generate individual treatment effects from normal distribution
    individual_effect = case_when(
      adheres_to_treatment ~ rnorm(n(), mean = 0.118, sd = 0.06),
      TRUE ~ 0
    ),
    
    # Ensure no weight gain from treatment by capping at 0
    individual_effect = pmax(individual_effect, 0),
...
saveRDS(full_results, "full_simulation_results7.rds")
 

countries<-full_results %>% select(ISO) %>% distinct()
...
full_results <- readRDS("full_simulation_results7.rds")
