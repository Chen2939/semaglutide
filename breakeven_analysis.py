...
# =====================================================================
# PART 2: Load mortality-side emissions (with population weighting)
# =====================================================================

def load_mortality_emissions():
    """Load year-by-year survivor emissions from Mortality Model output,
    scaled up by average population weighting.

    The mortality model's run_multi_simulation() sums alive/dead diffs
    across ~500 synthetic individuals per age-sex cell WITHOUT applying
    the population weighting column. The food model, by contrast, uses
    FAOSTAT national consumption (full population scale). To align the
    two sides, we multiply mortality outputs by the average weighting
    factor per country from the microsim.
    """
    mort = pd.read_csv("mortality model total emissions.csv")

    # Compute average weighting per (ISO, scenario) from the microsim
    microsim = pd.read_pickle("final_df_imputed.pkl")
    avg_wt = (
        microsim.groupby(["ISO", "scenario"])["weighting"]
        .mean()
        .reset_index()
        .rename(columns={"weighting": "avg_weighting"})
    )

    mort = pd.merge(mort, avg_wt, on=["ISO", "scenario"], how="left")
    # Countries without weighting data default to 1 (unscaled)
    mort["avg_weighting"] = mort["avg_weighting"].fillna(1.0)

    # Scale diff and emissions columns by population weight
    for y in range(0, 11):
        mort[f"diff_Y{y}"] = mort[f"diff_Y{y}"] * mort["avg_weighting"]
    mort["total_person_years_saved"] = mort["total_person_years_saved"] * mort["avg_weighting"]

    for y in range(1, 11):
        mort[f"emissions_Y{y}"] = mort[f"emissions_Y{y}"] * mort["avg_weighting"]
    mort["total_emissions"] = mort["total_emissions"] * mort["avg_weighting"]
...
def compute_breakeven(food_savings, mort):
    ...
    for _, row in merged.iterrows():
        ...
        records.append({
            ...
            "avg_weighting": row.get("avg_weighting", 1.0),
...
def main():
    ...
    print("""
  NOTE: Mortality-side values have been scaled by average population
  weighting from the microsim to align with population-level food savings.
  This correction should be validated with the research team — the raw
  mortality model CSV does not apply weighting in its aggregation.
""")
